...
from train_srgan import batch_to_tensor
...
def infer_shape(sample: object) -> tuple[int, ...]:
...
def summarize_dataset(data_dir: Path, batch_size: int = 512, max_events: int | None = None) -> dict:
    ...
    for path in files:
        ...
        for batch in parquet.iter_batches(
            ...
            lr = batch_to_tensor(batch.column(0)).to(torch.float32)
            hr = batch_to_tensor(batch.column(1)).to(torch.float32)
            ...
            for value in pt.tolist():
                pt_stats.update(float(value))
            for value in m0.tolist():
                m0_stats.update(float(value))
            ...
            for value in lr_sum.tolist():
                lr_energy.update(float(value))
            for value in hr_sum.tolist():
                hr_energy.update(float(value))
            for value in response.tolist():
                lr_ratio_stats.update(float(value))
            for value in (hr_sum / torch.clamp(lr_sum, min=1e-8)).tolist():
                hr_ratio_stats.update(float(value))
            for value in lr_nonzero.tolist():
                lr_sparsity.update(float(value))
            for value in hr_nonzero.tolist():
                hr_sparsity.update(float(value))

            for cls in torch.unique(y).tolist():
                mask = y == int(cls)
                if not mask.any():
                    continue
                for value in pt[mask].tolist():
                    pt_by_class[int(cls)].update(float(value))
                for value in m0[mask].tolist():
                    m0_by_class[int(cls)].update(float(value))
                for value in response[mask].tolist():
                    response_by_class[int(cls)].update(float(value))
    ...
    summary = {
        ...
        "image_shapes": {
            "lr": inferred_lr_shape,
            "hr": inferred_hr_shape,
        },
        "pt": {
            "mean": pt_stats.mean(),
            "std": pt_stats.std(),
...
def write_report(summary: dict, out_dir: Path) -> None:
    ...
    report_lines = [
        ...
        f"Low-resolution image shape: {summary['image_shapes']['lr']}",
        f"High-resolution image shape: {summary['image_shapes']['hr']}",
        "",
        "## Class balance",
        "",
        "| Class | Count |",
...
def main() -> None:
    ...
    parser.add_argument("--max-events", type=int, default=None, help="Optional cap for a faster sample-based analysis")
